<script lang="ts">
	...
	// Dashboard metrics are now handled by derivedMetrics

	// Dashboard metrics - will be calculated after data loads
	let metrics = {
		upcomingEvents: 0,
		pendingContracts: 0,
		followUpsNeeded: 0,
	...
	onMount(async () => {
		try {
			...
			// Calculate metrics using the store subscriptions
			let currentEventStore: any;
			let currentClientStore: any;
			let currentPerformerStore: any;

			eventStore.subscribe((store) => (currentEventStore = store))();
			clientStore.subscribe((store) => (currentClientStore = store))();
			performerStore.subscribe((store) => (currentPerformerStore = store))();

			metrics = {
				upcomingEvents: currentEventStore.events.filter(
					(e: any) => e.status !== 'completed' && e.status !== 'cancelled'
				).length,
				pendingContracts: currentEventStore.events.filter(
					(e: any) => !e.contract.sent || !e.contract.received
				).length,
				followUpsNeeded: currentClientStore.clients.filter(
					(c: any) => c.nextFollowUp && c.nextFollowUp.date
				).length,
				monthlyRevenue: calculateMonthlyRevenue(currentEventStore.events),
				activeClients: currentClientStore.clients.filter((c: any) => c.status === 'active').length,
				totalPerformers: currentPerformerStore.performers.length
			};
		} catch (error) {
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
	<div class="bg-white p-6 rounded-lg shadow-sm border-l-4 border-blue-500 mb-6">
		<div class="flex justify-between items-center">
			<div>
				<h1 class="text-2xl font-bold text-gray-900">Welcome to CircusSync</h1>
