...
<script lang="ts">
	...
	$: filteredEvents = $eventStore.events.filter((event) => {
		...
		if (searchTerm) {
			...
			return (
				...
				event.clientLiaison.toLowerCase().includes(search) ||
				event.gigManager.toLowerCase().includes(search)
			);
		}
	...
	function isEventUpcoming(date: Date): boolean {
		const today = new Date();
		return date >= today;
	}
		...
		<div class="flex flex-col md:flex-row md:items-center justify-between">
			...
			<div class="mt-4 md:mt-0 flex flex-col sm:flex-row gap-3">
              ...
              `}
					...
					>
						<Icon name="list" size={18} class="mr-1" />
						List
					</button>
					<button
						type="button"
              ...
              `}
					...
					>
						<Icon name="calendar" size={18} class="mr-1" />
						Calendar
					</button>
				</div>
...
<div class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
	...
	{:else if sortedEvents.length === 0}
		<div class="bg-white rounded-lg shadow-sm p-6 text-center">
			<Icon name="calendar" size={48} class="mx-auto text-gray-400 mb-4" />
			...
			{#if canManageEvents}
				<Button href="/events/new" class="mt-4">
					<Icon name="plus" size={18} class="mr-1" />
					Create an Event
				</Button>
			{/if}
							...
							<div class="px-4 py-4 sm:px-6">
								<div class="flex items-center justify-between">
									<div class="flex items-center">
										...
										<Badge color={getStatusColor(event.status)} class="ml-2">
								...
								<div class="mt-2 sm:flex sm:justify-between">
									<div class="sm:flex sm:flex-wrap">
										...
										<div class="mt-2 flex items-center text-sm text-gray-500 sm:mt-0">
											<Icon name="users" size={16} class="flex-shrink-0 mr-1.5 text-gray-400" />
											{event.performers?.length || 0} performers
										</div>
									</div>
									<div class="mt-2 flex items-center text-sm text-gray-500 sm:mt-0">
										<Icon name="dollar-sign" size={16} class="flex-shrink-0 mr-1.5 text-gray-400" />
										${event.payment.totalValue.toFixed(2)}
									</div>
								</div>
							</div>
						</a>
		...
		<div class="bg-white rounded-lg shadow">
			...
			<div class="grid grid-cols-7 gap-px bg-gray-200">
				...
				{#each Array(35) as _, i}
					...
					<div
						class={`min-h-24 bg-white p-1 ${isCurrentMonth ? '' : 'bg-gray-50'} ${
							isToday ? 'border-2 border-blue-500' : ''
						}`}
					>
						{#if dayNum > 0}
							<div class="text-right text-sm font-medium text-gray-700 p-1">
								{dayNum}
							</div>

							{#if dayNum === 6}
								<!-- Example event for today -->
								<div class="mt-1 px-2 py-1 rounded-md text-xs bg-blue-100 text-blue-800 truncate">
									YPO Yearly Celebration
								</div>
							{/if}

							{#if dayNum === 14}
								<!-- Example event -->
								<div class="mt-1 px-2 py-1 rounded-md text-xs bg-green-100 text-green-800 truncate">
									Pritzker Elementary Event
								</div>
							{/if}
							...
							{#if dayNum === 18}
								<!-- Example event -->
								<div
									class="mt-1 px-2 py-1 rounded-md text-xs bg-purple-100 text-purple-800 truncate"
								>
							...
							{#if dayNum === 26}
								<!-- Example event -->
								<div
									class="mt-1 px-2 py-1 rounded-md text-xs bg-orange-100 text-orange-800 truncate"
								>
									Jefferson Park Block Party
								</div>
							{/if}
						{/if}
					</div>
