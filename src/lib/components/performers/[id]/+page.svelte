...
<script lang="ts">
	...
	// User permissions
	const canEditPerformerStore = checkUserRole('manager');
	$: canEditPerformer = $canEditPerformerStore;
	...
	$: upcomingEvents = performerEvents.filter(
		(e) => new Date(e.date) >= new Date() && e.status !== 'cancelled'
	).length;

	$: pastEvents = performerEvents.filter(
...
{:else}
			...
			<div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
				...
				{#if canEditPerformer}
					<div class="flex flex-wrap gap-2">
						...
						<Button variant="outline" color="red" on:click={() => (showDeleteConfirm = true)}>
	...
	<div class="bg-white border-b">
		<div class="max-w-7xl mx-auto px-4">
			<div class="flex overflow-x-auto">
				...
				/>
				<Tab label="Notes" active={activeTab === 'notes'} on:click={() => (activeTab = 'notes')} />
			</div>
		</div>
	</div>
			...
			<div>
				...
				{#if !performer.skills || performer.skills.length === 0}
					<div class="bg-white rounded-lg shadow p-8 text-center">
						...
						{#if canEditPerformer}
							<Button on:click={() => (showEditModal = true)} class="mt-4">Add First Skill</Button>
						{/if}
					</div>
				{:else}
			...
			<div>
				...
				{#if performerEvents.length === 0}
					<div class="bg-white rounded-lg shadow p-8 text-center">
						...
						{#if canEditPerformer}
							<Button href="/events/new?performer={performer.id}" class="mt-4">Add to Event</Button>
						{/if}
					</div>
				{:else}
										...
										<div class="px-4 py-4 sm:px-6">
											...
											<div class="mt-2 sm:flex sm:justify-between">
												...
												{#if event.performers.find((p) => p.performer === performerId)}
													...
													<div
														class="mt-2 flex items-center text-sm font-medium text-green-700 sm:mt-0"
													>
														<Icon
															name="dollar-sign"
															size={16}
															class="flex-shrink-0 mr-1.5 text-green-600"
														/>
	...
	<!-- Modals -->
	{#if showEditModal}
		<Modal title="Edit Performer" size="lg" on:close={() => (showEditModal = false)}>
			<PerformerForm
				{performer}
