...
<div class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
	...
	{:else if sortedPerformers.length === 0}
		<div class="bg-white rounded-lg shadow-sm p-6 text-center">
			<Icon name="user" size={48} extraClass="mx-auto text-gray-400 mb-4" />
			...
			{#if canAddPerformer}
				<Button on:click={() => (showAddPerformerModal = true)} className="mt-4">
					<Icon name="plus" size={18} extraClass="mr-1" />
				...
				<div class="bg-white shadow overflow-hidden rounded-lg">
					...
					{#if performer.skills && performer.skills.length > 0}
						<div class="border-t border-gray-200 px-4 py-4 sm:px-6 bg-gray-50">
							<h4 class="text-xs text-gray-500 uppercase tracking-wide font-semibold mb-2">
								Skills
							</h4>
							<div class="flex flex-wrap gap-1">
								{#each performer.skills as skill}
									<div class="flex items-center">
										<span
											class="inline-block px-2 py-1 text-xs rounded-full {`bg-${getColorForSkill(
												skill.category
											)}-100 text-${getColorForSkill(skill.category)}-800`}"
										>
											{formatSkillCategory(skill.category)}
										</span>
										{#if skill.level}
											<span
												class="ml-1 inline-block px-2 py-1 text-xs rounded-full {`bg-${getColorForLevel(
													skill.level
												)}-100 text-${getColorForLevel(skill.level)}-800`}"
											>
...
{#if showAddPerformerModal}
	<Modal title="Add New Performer" size="lg" on:close={() => (showAddPerformerModal = false)}>
		<PerformerForm
